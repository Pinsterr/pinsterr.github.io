<template>
  <v-card color="grey lighten-4" light>
    <v-card-text class="text-xs-center">
      <h4>
        OK ... OK ... My CV has just been
        finished here but not my journey!
      </h4>
      <div>
      ...
      <div v-if="toggleMessage" class="ma-4">
        <span class="pre">{{
          message.text
        }}</span>
        <div
...
export default {
  ...
  data: () => ({
    ...
    messages: [
      ...
      {
        text:
          "We all should be curious about it, shouldn't we?",
        from: 4,
        to: 4,
      },
      {
        text:
          'With a plan and persistence ...',
        from: 5,
        to: 5,
      },
      {
      ...
      {
        text:
          'Of course with a lot of hardship ... a LOT OF them! ...',
        from: 9,
        to: 9,
      },
      {
    ...
    loadFuture: debounce(function() {
      ...
      setTimeout(() => {
        ...
        if (!this.message)
          ...
          this.message.to !== null &&
          this.message.to < this.futurePage
        )
          this.message = this.messages.find(
            (msg) => {
              return (
                msg.from === this.futurePage
              )
            }
          )
